...
- 明确标记垃圾、采纳检测拉黑、页面批量处理和误标纠错分别记录，不再把引擎命中或普通白名单直接当作训练标签。
- 用户操作与证据事件同次本地存储，独立离线队列按事件 ID 补交；保留原文、昵称、简介、时间、帖子 ID、版本及截断信息。尊重「名单上传」开关，不增加社区票。
- 后端新增不可覆盖的样本事件和追加式人工复核历史；管理后台支持查看原始证据、审定正反例、语义标签、模板家族和固定版本导出。
- 增加按时间、家族、账号、同帖和内容关联隔离的数据集构建与逐家族评测工具，未审、冲突、不确定或材料不足的记录不进入评测。
- 修复误标反馈拒收 `keyword:*` 规则 ID，以及侧栏纠错丢失原始检测证据的问题；修复 X 复用卡片时手动按钮可能沿用旧证据的问题。
- 本版先部署数据层，不启用语义模型或自动训练；插件由维护者自行打包。
...
- 官网词库 Tab 默认全量展开短语；黑名单与社区抢救表改为分页（黑名单 100 条/页、抢救 50 条/页），搜索时自动回到第一页。
